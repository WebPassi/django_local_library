...
\\chead{" + mitte +"} \n"
    if rechts == "":
        latexstr+="\\rhead{\includegraphics[scale=0.5]{logo.png}} \n"
    else:
        latexstr+="\\rhead{" + rechts + "} \n"
...
def preamble(links="\\today",mitte="",rechts=""):
...
def zertifikat(title,klasse):
    latexstr=open("zertifikate/zertifikat.tex").read()
    return latexstr
...
def makeExercise(aufgabe,punkte="",aufgabentext=""):
    if punkte == "":
        latexstr="\\begin{aufgabe} ~ \\\\ \n"
        latexstr+=aufgabentext
        latexstr+=aufgabe
...
def makeSheet(exercises="",title="",klasse=""):
    latexstr=""
    latexstr+=preamble(title,klasse)
...
def makeTest(exercises="",kurs="",title="",punkte="",zeit=45):
    
    latexstr=preamble(mitte=kurs)
    latexstr+="\\begin{document} \n\
    \\begin{flushleft}\n"

    latexstr+="\\renewcommand{\\arraystretch}{2.15} \n\
\\begin{tabular}{|p{10cm}|p{2cm}|p{2cm}|p{2cm}|} \n\
\\hline \n\
\\hspace{2cm} Punkte von \\qquad" + str(punkte) + "\\qquad Punkten erreicht & \\hspace{1.2cm} NP & G & E \\\\ \n\
\\hline \n\
...
def bruch(expr):
    ...
    myexpr=expr
    xterm=list(range(len(summanden)))
    zfaktor=list(range(len(summanden)))
    nfaktor=list(range(len(summanden)))
    newsummand=list(range(len(summanden)))
    oldsummand=list(range(len(summanden)))
...
def multicol(inhalt,spalten=3):
    if spalten == 1:
        return enumlist(inhalt)
    ...
    if dars=="allinone":
        ...
        for f in funktionsliste:
            ...
            if name=="y0":
                ### Parabel
                #latexstr+="\\draw (" + latex(f.d+0.5) + "," + latex(f.e+0.2) + ") node[right] {" + "$\\scriptscriptstyle " + latex(f.name) + "$}; \n"
                latexstr+="\\draw (" + latex(0.5) + "," + latex(0.5) + ") node[right] {" + "$\\scriptscriptstyle " + latex(f.name) + "$}; \n"
...
def my_latex(ausdruck):
    ausdruck=str(ausdruck)
    ausdruck_latex=ausdruck.replace('**','^').replace(' ','').replace('.',',').replace('*','\\cdot ')
    #ausdruck_latex="$" + ausdruck.replace('**','^').replace('*','').replace(' ','') + "$"
        
    return ausdruck_latex
