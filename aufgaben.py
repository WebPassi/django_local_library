...
##### 7. Klasse
def aufg_zahlen_ordnen(zahlen,aufgabe="Ordne die folgenden Zahlen der Gr\\\"o\\ss{}e nach. Beginne mit der kleinsten Zahl."):
    aufgabe += "\\["

    for z in zahlen:
        aufgabe += latex(z) + "\\quad ; \\quad"
    aufgabe += "\\]"

    aufgabe += "\\underline{\\hspace{12cm}}"

    return aufgabe

def aufg_zahlen_ordnen_lsg(zahlen):
    aufgabe="Ordne die folgenden Zahlen der Gr\\\"o\\ss{}e nach. Beginne mit der kleinsten Zahl."
    aufgabe += "\\["
...
def erstelleLG_alt(form,zahlenbereich,ug,og):
    if zahlenbereich=="Z":
        if form=="einfach":
            ...
            return glg.zeigeGlg()
### lineare Gleichungen
def aufgGlg(term1,term2):
    glg=Equation(term1,term2)
    return glg.zeigeGlg()

def aufgGlg_lsg(term1,term2):
    glg=Equation(term1,term2)
    return glg.zeigeLsg()
...
def aufgPythagoras(a="",b="",c="",alpha="",beta="",gamma=""):
    aufgabe = "$"
    if not a == "":
        aufgabe += "a=" + str(a) + " \\mathrm{cm} \\\\"
    if not b == "":
        aufgabe += "b=" + str(b) + " \\mathrm{cm} \\\\"
    if not c == "":
        aufgabe += "c=" + str(c) + " \\mathrm{cm} \\\\"
    if not alpha == "":
        aufgabe += "\\alpha=" + str(alpha) + "^{\\circ} \\\\"
    if not beta == "":
        aufgabe += "\\beta=" + str(beta) + "^{\\circ} \\\\"
    if not gamma == "":
        aufgabe += "\\gamma=" + str(gamma) + "^{\\circ} \\\\"
...
def aufgTrigonometrie(a="",b="",c="",alpha="",beta="",gamma=""):
    aufgabe = "$"
    if not a == "":
        aufgabe += "a=" + str(a) + " \\mathrm{cm} \\\\"
    if not b == "":
