...
### Die Präambel einer Latex-Datei
### Hier können grundlegende Einstellungen des Layouts getroffen werden.
def preambel():
...
### Erstellt aus dem übergebenen Parameter ein Latex-Dokument
### Eingabe und Ausgabe: Latex-Ausdruck
...
##### Zahlen
### Gibt die Summanden eines Polynoms als Liste zurück 
def zerlegung(expr):
    sum=expr.split(r"[+,-]")
    return sum
...
### Nummeriert die übergebenen Funktionen und benennt sie entsprechend (z.B. f_1,...) 
...
def erstelleNamensliste(funktionsliste,name="f"):
    ...
    for index,funktion in enumerate(funktionsliste):
        funktion.name=latex(name) + "_" + latex(index+1)
        enumfunktion[index]=latex(funktion.name)
    return enumfunktion

### Darstellung von den Lösungen einer Gleichung
...
### Rundet eine Zahl auf die gewünschte Nachkommastelle
...
##### Listen, Tabellen und minipage

### Ruft die multicols-Umgebung von Latex auf
### Eingabe: inhalt als Liste von Latex-Ausdrücken, spaltenanzahl
...
### Eingabe: inhalt als Liste von Latex-Ausdrücken
def enumlist(inhaltsliste):
    ...
    latex_str+='\\end{enumerate} \n'
    return latex_str
### Eine Tabelle mit Gitter ohne Einträge
def plain_table(n,m):
    x=list(range(n))
    y=list(range(m))
...
### Erstellt eine Latex-Tabelle aus der übergebenen Matrix
def table(matrix):
...
### Notwendige Ersetzungen für die Skalierung der Achsen 
def pgflist(a,b):
