...
def ab_normalparabel():
    ...
    #numfunc=enumlist(erstelleFunktionsliste(flist))
    aufg2=makeExercise(aufgParabeleigenschaftenLSG(flist))
    aufg3=makeExercise(aufgFunktionaufstellenLSG(flist))
    ...
    for index,funktion in enumerate(flist):
        funktion.name="f_" + latex(index+1) 
    aufg4=makeExercise(aufgGraphLSG(flist))

    
    sheet=makeSheet(aufg1+aufg2+aufg3+aufg4)
