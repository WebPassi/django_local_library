...
\draw (2.5,1.2) node[right] {$\scriptscriptstyle f_1$}; 
...
$f_1$ & $S(2|1)$ & $x=2$ & Keine Nullstelle\\ \hline 
$f_2$ & $S(-1|0)$ & $x=-1$ & $x=-1$\\ 
...
\begin{scope} 
\clip (-3,-3) rectangle (3,3); 
\draw[color=black] plot[smooth] function{x**2 - 2*x + 1}; 
\draw (1.5,0.2) node[right] {$\scriptscriptstyle f$}; 
\end{scope} 
