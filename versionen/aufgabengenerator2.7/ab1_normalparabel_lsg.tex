...
$f_1$ & $S(0|1)$ & $x=0$ & Keine Nullstelle\\ \hline 
$f_2$ & $S(-2|-2)$ & $x=-2$ & $x_1\approx -3,\hspace{-1pt}41\quad x_2\approx-0,\hspace{-1pt}59$\\ \hline 
...
$f_2(x)=x^{2} + 1$\\[1em] 
$\cancel{f_3(x)=\left(x + 2\right)^{2}}$\\[1em] 
...
\draw[color=black] plot[smooth] function{(x - 1)**2 + 2}; 
\draw (1.5,2.2) node[right] {$\scriptscriptstyle f_5$}; 
\draw[color=black] plot[smooth] function{(x + 1)**2 + 2}; 
\draw (-0.5,2.2) node[right] {$\scriptscriptstyle f_6$}; 
\end{scope} 
\end{tikzpicture}\end{aufgabe} 
\begin{aufgabe} ~ \\ 
